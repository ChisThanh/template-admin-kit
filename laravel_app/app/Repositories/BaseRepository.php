...
abstract class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function searchTextCount($text, array $columns, array $columnsHas, array $join, $withTrashed = false)
    {
        if (!empty($text)) {
            ...
            return $query->distinct()->count($this->model->getTable() . '.' . $this->model->getKeyName());
        }
    ...
    public function countByColumn(array $columns, array $columnsHas, array $join, $withTrashed = false)
    {
        ...
        if (!isset($query)) {
            if ($withTrashed) {
                return $this->model->distinct()->withTrashed()->count($this->model->getKeyName());
            } else {
                return $this->model->distinct()->count($this->model->getTable() . '.' . $this->model->getKeyName());
            }
        }
    ...
    public function searchTextCountGroup($text, array $columns, array $columnsHas, array $join, array $group_by = [],  $withTrashed = false)
    {
        if (!empty($text)) {
            ...
            $query = $this->generateColumn($query, $columns);
            $query = $this->generateHasColumn($query, $columnsHas);
            return $query->groupBy($group_by)->distinct()->count($this->model->getTable() . '.' . $this->model->getKeyName());
        }
    ...
    public function countByColumnGroup(array $columns, array $columnsHas, array $join, array $group = [],  $withTrashed = false)
    {
        ...
        return $query->groupBy($group)->distinct()->count($this->model->getTable() . '.' . $this->model->getKeyName());
    }
                ...
                foreach ($joins as $join) {
                    ...
                    if (!isset($query)) {
                        if (isset($join['where']) && count($join['where']) > 0) {
                            $query = $this->model->$func($join['join_table'], function ($j) use ($join) {
                                $j->on($join['join_column'], $join['join_cond'], $join['main_column']);
                        ...
                        if (isset($join['where']) && count($join['where']) > 0) {
                            $query->$func($join['join_table'], function ($j) use ($join) {
                                $j->on($join['join_column'], $join['join_cond'], $join['main_column']);
                                foreach ($join['where'] as $sql) {
                                    $j->whereRaw($sql);
                                }
    ...
    {
        $prefix_length = $length > 1 ? $length : 1;
