...
abstract class BaseApiController extends AppBaseController
{
    ...
    public function searchMelisearch(Request $request){
        $inputs = $request->all();
        $resp = $this->service_base->searchMelisearch($inputs);
        if($resp['code'] !== '200'){
            return $this->sendError($resp['message'], $resp['code']);
        }
        return $this->sendResponse($resp['data'], 'Search success');
    ...
    public function getDictByIdAndConds(Request $request){
        $inputs = $request->all();
        $resp = $this->service_base->getDictByIdAndConds($inputs);
        if($resp['code'] !== '200'){
            return $this->sendError($resp['message'], $resp['code']);
        }
        return $this->sendResponse($resp['data'], 'Get dict by id and conditions success');
    }

    public function getDictByObjectIdAndConds(Request $request){
        $inputs = $request->all();
        $resp = $this->service_base->getDictByObjectIdAndConds($inputs);
        if($resp['code'] !== '200'){
            return $this->sendError($resp['message'], $resp['code']);
        }
        return $this->sendResponse($resp['data'], 'Get dict by object, ids and conditions success');
    }
